...
const int PULSE_LED = 1;           // Turn on/off the LED [1,0]
...
const int   signalPin        = A0;      // The signal input pin
const float signalThreshold  = 80;      // Signal threshold, measured from 0-1023. 80 corresponds to approximately 20 mV threshold. 
...
Serial.println("###   Counts Time[ms] Amplitude[V] Pulse[mV] DeadTime[ms]    ###");
...
void loop() {
  while (1) {
    if (analogRead(A0) > signalThreshold) 
        ...
        float a = -0.0000213241;
        float b =  0.0236566939;
        float c = -(0.3378053306 + ampV);
        pulseV = (-b + sqrt(sq(b) - 4*a*c))/(2*a);
        
        Serial.println((String)count + " " + (String)initial + " " + (String)ampV + " " + (String)pulseV + " " + (String)(18. + timeOLED));
        
        timeOLED = 0;                      // Reset the OLED screen deadtime
        interrupts();                      // Allow for interupts 
...
void getTime(){
  ...
  minutes             = time / 60 % 60;
  hours               = time / 3600;
  stdev               = sqrt(count) / time;
  average             = (float)count / time;
